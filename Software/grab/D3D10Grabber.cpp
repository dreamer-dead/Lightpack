...
public:
    ...
    void setCallback(GetHwndCallback_t callback)
    {
        m_getHwndCb = callback;
    }
    ...
    GrabResult grab(QList<QRgb> *grabResult, QList<GrabWidget *> *grabWidgets)
    {
        if (!m_isInited) {
...
private:
    ...
    QTimer * m_processesScanAndInfectTimer;
    bool m_isInited;
...
GrabResult D3D10Grabber::_grab() {
    ...
    return m_impl->grab(m_grabResult, m_grabWidgets);
}
